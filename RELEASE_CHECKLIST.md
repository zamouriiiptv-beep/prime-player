...
- [ ] **Castivio starts silently.** There is no startup sound and no audio asset:
      the mark is 1.2 seconds of picture and nothing else, so there is no volume
      rule to get wrong, no third-party audio licence to clear, and nothing that
      plays over whatever the user was already listening to. `CastivioIntro` owns
      the whole of it and `IntroTest` pins its timeline.
- [ ] **Exit confirmation everywhere Back can leave.** From the shell's root, from
      Add a playlist and from the licence screen — every path that calls
      `finish()` goes through `ExitGuard`, which is the fix for the version that
      lived in the shell and so could not be reached on a fresh install. Back
      inside the app does not ask, back while it is open closes only the dialog,
      and focus starts on Cancel. `BackPolicyTest` holds the shell's ladder; the
      dialog is `CastivioDialog`, shared with the legal notice through one panel.
